 ...
 * @version $Id$
 */
public abstract class GTExpression extends SearchExpression {
    public GTExpression(String propertyName, Class propertyType) {
        super(propertyName, propertyType);
    }

    public Criterion getCriterion(Map<String, String> aliases) {
        return Restrictions.gt(resolvePropertyName(aliases), getValue());
    }
}
